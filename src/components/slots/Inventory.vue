  ...
  <div class="inventory_slot">
    <div
      ...
      v-bind:style="{
        ...
        backgroundPosition: `right ${(getItem(item).graphics.column * 32) + 32}px top 0`
      }">
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getItem(item) {
      return this.library.find(i => i.itemID === item.itemID);
    },
  ...
  computed: {
    // TODO
    // Extract extranous objects and only have IDs
    images() {
